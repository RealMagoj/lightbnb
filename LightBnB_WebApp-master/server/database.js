...
const getAllReservations = function(guest_id, limit = 10) {
  return pool.query(`SELECT * FROM reservations 
                    ...
                    WHERE users.id = $1 LIMIT $2;`, [guest_id, limit])
    .then((result) => {
      return result.rows;
    })
    .catch((err) => {
...
const getAllProperties = function(options, limit = 10) {
  ...
  if (options.owner_id) {
    queryParams.length > 0 ? queryString += `AND ` : `WHERE `
  ...
  if (options.minimum_price_per_night && options.maximum_price_per_night) {
    ...
    const minimum_price_per_night = queryParams.length;
    queryParams.push(options.maximum_price_per_night * 100);
    queryString += `(cost_per_night >= $${minimum_price_per_night} AND cost_per_night <= $${queryParams.length}) `
  }
